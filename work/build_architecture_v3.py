...
ROOT=Path.cwd();SPEC=ROOT/'work/buildings/v3/inventory.json';PROFILES=json.loads((ROOT/'work/buildings/v3/profiles.json').read_text());OUT=ROOT/os.environ.get('ARCHITECTURE_OUT','outputs/orthogonal-multiview');OUT.mkdir(parents=True,exist_ok=True);DATA=json.loads(SPEC.read_text())['buildings'];SELECT=set(filter(None,os.environ.get('BUILD_IDS','').split(',')))
