...
本轮根据“住宅方正，整体颜色和纹理应与照片对齐”的要求重建建筑。保留原红框建筑 B001，重新生成其余 **{c["new_units"]} 个编号单元**；编号包含主屋、连接侧翼与小型附属房，不等同于独立住户数。
...
| 有高度重叠的住宅主体之间穿插 | 0 对 |
| 原红框建筑 | 681 个对象的几何校验值保持一致 |
| 新建筑对象 | {c['new_objects']:,} |
| 新建筑网格顶点 | {c['mesh_vertices']:,} |
| 实际开洞（包括门、窗与阳台） | {c['openings']:,} |
| 外部链接库 | 0 |
| 所需图片 | {c['packed_images']} 张已打包；墙面与屋顶细节主要由连续材质生成 |
...
**照片能够提供的细节并不均等。** 近景可识别部分已单独修整，远景与遮挡面使用规则门窗、估计层高和推测构件。栏杆数量、窗框截面、未见立面、室内、细小设备不能视为实测复原。道路、地形、车辆和植物背景延用原近似场景，其中照片地面投影在偏离参考视角时仍有拉伸和拼接；建筑对照卡将各房屋单独显示，便于直接判断本轮建筑修改。
...
文件打开时提供整村相机视图。使用原视频恢复相机 `Video 044.00s | recovered` 对照近景；返回段另有 256 秒相机。渲染使用 Cycles；材质预览用于查看颜色、砖缝和旧化。
