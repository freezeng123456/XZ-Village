...
ROOT=Path.cwd();SPEC=ROOT/'work/buildings/v2/inventory.json';PROFILES=json.loads((ROOT/'work/buildings/v2/profiles.json').read_text());OUT=ROOT/os.environ.get('ARCHITECTURE_OUT','outputs/rectangular-photo-refinement');OUT.mkdir(parents=True,exist_ok=True);DATA=json.loads(SPEC.read_text())['buildings'];SELECT=set(filter(None,os.environ.get('BUILD_IDS','').split(',')))
...
PARENT=old[0] if partial and old else bpy.data.collections.new('11 ARCHITECTURE | rectangular source-refined buildings')
...
WHITE=material('V2 / weathered ivory architectural trim',[.77,.77,.71]);DARK=material('V2 / deep reveal shadow',[.055,.060,.052]);FRAME=material('V2 / aged aluminium frames',[.20,.23,.22],.42,.55);STEEL=material('V2 / stainless tank and bracing',[.60,.64,.64],.30,.78);RUST=material('V2 / oxidized roof bracing',[.29,.26,.21],.73,.35);GLASS=material('V2 / dark green reflective glazing',[.15,.225,.20],.24,.28);WOOD=material('V2 / aged timber doors',[.28,.215,.14],.85);MORTAR=material('V2 / worn concrete plinth',[.44,.45,.40]);SOLAR=material('V2 / photovoltaic cells',[.11,.17,.22],.28,.45);BLUE=material('V2 / painted sheet roof',[.17,.49,.61],.52,.25)
def weather_material(name,color,style,age,axis=0,H=10,stains=None,wall_length=10):
 ...
 dirt=mathnode('MULTIPLY',mathnode('ADD',mathnode('MULTIPLY',patch.outputs['Color'],.38),mathnode('MULTIPLY',streak.outputs['Fac'],wet)),age)
 if stains:
  for stain in stains:
   xx=mathnode('DIVIDE',mathnode('SUBTRACT',sep.outputs['X' if axis==0 else 'Y'],stain['x']*wall_length),stain['rx']*wall_length);zz=mathnode('DIVIDE',mathnode('SUBTRACT',sep.outputs['Z'],stain['z']*H),stain['rz']*H);radius=mathnode('ADD',mathnode('MULTIPLY',xx,xx),mathnode('MULTIPLY',zz,zz));zone=mathnode('MAXIMUM',mathnode('SUBTRACT',1,radius),0);speckle=mathnode('MULTIPLY',patch.outputs['Color'],stain['strength']*2.2);dirt=mathnode('MINIMUM',mathnode('ADD',dirt,mathnode('MULTIPLY',zone,speckle)),.92)
 if style in ('ceramic','ceramic_horizontal','brick'):
  tiles=N.new('ShaderNodeTexBrick');tiles.inputs['Scale'].default_value=1;tiles.inputs['Brick Width'].default_value=.235 if style=='brick' else (.30 if style=='ceramic_horizontal' else .10);tiles.inputs['Row Height'].default_value=.075 if style=='brick' else (.095 if style=='ceramic_horizontal' else .20);tiles.inputs['Mortar Size'].default_value=.004 if style=='brick' else .0011;tiles.offset=.5 if style=='brick' else 0;tiles.inputs['Color1'].default_value=(*(rgb*.90),1);tiles.inputs['Color2'].default_value=(*(rgb*1.05),1);tiles.inputs['Mortar'].default_value=(*(rgb*.60),1);L.new(coord.outputs[0],tiles.inputs['Vector']);surface=tiles.outputs['Color'];bumpheight=tiles.outputs['Fac']
 else:
...
class Builder:
 ...
 def box(self,name,center,dims,mat,basis=None):
 ...
 def finish(self):
  ...
  self.col['opening_count']=self.openings;self.col['source_frame_seconds']=self.b['t'];self.col['rectangular_core']=True;self.col['width']=self.W;self.col['depth']=self.D;self.col['roof_height']=self.b['roof_height'];self.col['estimated_ground_height']=self.b['base_height'];self.col['original_component_count']=self.components;self.col['status']='Source-refined rectangle; visual review pending';return dict(id=self.id,objects=len(self.col.objects),components=self.components,vertices=sum(len(o.data.vertices) for o in self.col.objects if o.type=='MESH'),openings=self.openings)

def make_building(b):
 B=Builder(b);p=PROFILES[b['id']];W,D,H=B.W,B.D,B.H;n=b['floors'];sh=H/n;seed=int(hashlib.sha256(b['id'].encode()).hexdigest()[:8],16);rng=np.random.default_rng(seed);wallmats=[weather_material(b['id']+f' / facade {j} {f["style"]}',f['color'],f['style'],f['age'],j%2,H,f.get('stains'),[W,D,W,D][j]) for j,f in enumerate(p['facades'])];roofmat=weather_material(b['id']+' / weathered roof finish',p['roof_color'],'paint',p.get('roof_age',.17),2,H);parapet=weather_material(b['id']+' / stained parapet concrete',[.73,.74,.68],'aged_plaster',.65,0,H+.5);wood=weather_material(b['id']+' / timber grain',[.34,.27,.17],'aged_plaster',.18,0,2.2)
 accent=material(b['id']+' / source burgundy trim',p['accent_color']) if p.get('accent_color') else WHITE
 glass=material(b['id']+' / source green glazing',p['glazing_color'],.26,.25) if p.get('glazing_color') else GLASS
 B.box('Ground floor closed slab',(W/2,D/2,.08),(W,D,.16),MORTAR)
 for floor in range(1,n+1):
  if b['type']!='construction':B.box('Structural floor slab',(W/2,D/2,floor*sh-.10),(W,D,.2),roofmat)
 corners=np.array([[0,0,0],[W,0,0],[W,D,0],[0,D,0]],float)
 for j,(a,z) in enumerate(zip(corners,np.roll(corners,-1,axis=0))):
  u=z-a;L=np.linalg.norm(u);u/=L;out=np.array([u[1],-u[0],0]);basis=np.array([u,out,[0,0,1]]);f=p['facades'][j];wm=wallmats[j]
  def at(x,zz,depth=0):return a+u*x+out*depth+[0,0,zz]
  def box(name,x,zz,ww,hh,depth,thickness,mat):B.box(name,at(x,zz,depth),(ww,thickness,hh),mat,basis)
  holes=[];loggias=[]
  ...
  if j==0 and L>2.1 and not b.get('public_hall'):
  ...
  for x0,x1 in zip(xs[:-1],xs[1:]):
   for z0,z1 in zip(zs[:-1],zs[1:]):
    ...
    box('Wall panel with true opening',(x0+x1)/2,(z0+z1)/2,x1-x0,z1-z0,-.12,.24,wm)
  for x0,x1,z0,z1,kind in holes:
   B.openings+=1;cx=(x0+x1)/2;cz=(z0+z1)/2;ww=x1-x0;hh=z1-z0
   if b['type']=='construction' or kind in ('loggia','hall'):continue
   if kind=='door':
    ...
    if recess>.20:
     ...
     B.box('Deep recessed window sill',at(cx,z0-.03,-recess/2),(ww,recess,.09),WHITE,basis)
    ...
    box('Projecting stone window sill',cx,z0-.055,ww+.20,.11,.07,.37,WHITE)
   for xx in [x0-.045,x1+.045]:box('Window or door outer vertical reveal',xx,cz,.09,hh+.12,.015,.11,WHITE if p['style']!='brick' else MORTAR)
   box('Window or door head lintel',cx,z1+.04,ww+.19,.10,.01,.13,WHITE if p['style']!='brick' else MORTAR)
  for x0,x1,z0,z1,log in loggias:
   ...
   for xx in np.linspace(x0,x1,log.get('columns',2)):
    box('Straight balcony supporting column',xx,(z0+z1)/2,.20,hh,.015,.24,WHITE);box('Balcony column capital',xx,z1-.08,.34,.16,.015,.36,WHITE);box('Balcony column foot',xx,z0+.12,.29,.22,.015,.30,WHITE)
   if log.get('arched'):
    positions=np.linspace(x0,x1,25);verts=[]
    for xx in positions:
     drop=.10+.32*(abs((xx-cx)/(ww/2))**2);verts.extend([at(xx,z1,.07),at(xx,z1-drop,.07)])
    B.mesh('Shallow curved balcony soffit',verts,[(2*k,2*k+1,2*k+3,2*k+2) for k in range(24)],WHITE)
   railz=z0+.96
   if p['baluster']=='stone':
    box('Stone balcony rail coping',cx,railz,ww+.14,.13,.055,.22,accent);box('Stone balcony rail foot',cx,z0+.15,ww,.15,.04,.22,WHITE)
    for xx in np.arange(x0+.20,x1-.12,.24):
     # Turned profiles give actual baluster silhouettes, not printed railing stripes.
     for lo,hi,rad in [(.22,.35,.052),(.35,.48,.034),(.48,.66,.056),(.66,.81,.033),(.81,.90,.052)]:B.cylinder('Turned concrete balcony baluster',at(xx,z0+(lo+hi)/2,.05),rad,hi-lo,WHITE,12)
   else:
  ...
  if b['type']!='gable':
   for floor in range(1,n):
    box('Storey band projecting coping',L/2,floor*sh,L,.12,.04,.12,WHITE if p['style']!='brick' else MORTAR)
   box('Ground damp plinth',L/2,.22,L,.44,.018,.09,MORTAR)
  if b['type'] in ('flat','solar','metal','hip'):
   levels=[(0,.28,.14),(.15,.40,.12)] if p['cornice']=='double' else [(-.16,.25,.12),(0,.38,.16),(.16,.47,.12)]
   for dz,th,hh in levels:box('Layered roof cornice',L/2,H+dz,L+.16,hh,.03,th,WHITE)
   if p['cornice']=='dark_band':box('Dark roof fascia band',L/2,H-.24,L,.10,.03,.27,FRAME)
  if b['type'] in ('flat','solar'):
   box('Weathered roof parapet',L/2,H+.43,L,.53,-.03,.19,parapet);box('Pale parapet cap',L/2,H+.71,L+.08,.085,-.025,.27,WHITE)
  # Drainage follows a real wall corner; repeated brackets and outlet bend are editable.
  if j in (0,2):
   xx=L-.19;B.line('Rainwater downpipe',[at(xx,H+.22,.15),at(xx,.35,.15),at(xx,.23,.30)],.040,WHITE)
  ...
  acs=[x for x in p.get('ac_windows',[]) if x['edge']==j]
  if not acs and b['type'] not in ('gable','construction') and L>5 and j==0:acs=[dict(x=.16,z=((n-1)*sh+.50)/H)]
  for ac in acs:
   cx=ac['x']*L;cz=max(.8,ac['z']*H)
   if any(cx-.44<r and cx+.44>l and cz-.30<t and cz+.30>bt for l,r,bt,t,k in holes):continue
   box('Exterior air conditioner housing',cx,cz,.78,.55,.26,.43,WHITE)
   ...
   B.line('Air conditioner cable and drain',[at(cx+.39,cz,.25),at(cx+.55,cz-.1,.18),at(cx+.55,cz-.95,.15)],.016,WHITE)
 # Roof finish and equipment.
 if b['type'] in ('flat','solar'):
  if p.get('roof_pavers')=='hex':
   tiles=[material(b['id']+f' / hexagonal roof paver {i}',np.clip(np.array(p['roof_color'])*f,.1,.95)) for i,f in enumerate([.84,.96,1.08,1.16])];radius=.255
  ...
  B.box('Roof drain grating',(W-.40,.42,H+.04),(.24,.24,.045),FRAME)
  for y in np.linspace(.34,.50,5):B.line('Drain grate slot',[(W-.51,y,H+.065),(W-.29,y,H+.065)],.008,DARK)
  if p.get('roof_partition'):
   y=D*p['roof_partition'];B.box('Source roof terrace dividing parapet',(W/2,y,H+.27),(W-.26,.18,.54),parapet);B.box('Terrace divider cap',(W/2,y,H+.56),(W-.20,.25,.07),WHITE)
  if b['type']=='solar':
 ...
 for tank in p.get('tanks',[]):
  ...
  B.line('Water tank feed pipe',[(x+rad,y,bottom+.22),(x+rad+.18,y,bottom+.22),(x+rad+.18,y,H+.12),(x+rad+.18,.25,H+.12)],.022,WHITE)
 if p.get('roof_utility_box'):
  u=p['roof_utility_box'];x=u['x']*W;y=u['y']*D;B.box('Source rooftop utility box',(x,y,H+.65),(1.0,.90,1.3),parapet);B.box('Utility box cover',(x,y,H+1.32),(1.16,1.06,.08),WHITE);B.line('Roof utility vent pipe',[(x,y,H+1.4),(x,y,H+2.4),(x+.28,y,H+2.4)],.065,WHITE)
 elif b['type'] in ('flat','solar') and not p['tanks']:
  B.cylinder('Roof ventilation upstand',(W*.22,D*.78,H+.25),.13,.50,parapet);B.cylinder('Roof vent cap',(W*.22,D*.78,H+.52),.19,.06,WHITE)
 return B.finish()
ledger=[r for r in json.loads((OUT/'building_ledger.json').read_text()) if r['id'] not in SELECT] if partial else []
for b in DATA:
 r=make_building(b);ledger.append(r);print('V2_BUILT',r,flush=True)
# Keep source context and the original landmark, while removing unreferenced V1 materials.
bpy.data.orphans_purge(do_local_ids=True,do_linked_ids=True,do_recursive=True)
s.view_layers[0].material_override=None;s.render.engine='CYCLES';s.cycles.device='CPU';s.cycles.samples=32;s.cycles.use_denoising=True;s.view_settings.view_transform='AgX';s.view_settings.look='AgX - Medium High Contrast';s.view_settings.exposure=.1
s['V2_scope']='Every residential core constrained to a rectangle; source-aware facade, balcony, roof equipment and weathering profiles. Hidden details remain inferred.'
for im in bpy.data.images:
 if im.source=='FILE' and not im.packed_file:im.pack()
(OUT/'building_ledger.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2));bpy.context.preferences.filepaths.save_version=0;bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Village_Rectangular_Candidates.blend'),compress=True);print('V2_CANDIDATE_SAVED',len(ledger),flush=True)
