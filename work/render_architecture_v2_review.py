"""Render source-camera crops and clean opposing views of each saved V2 building."""
import bpy,json,os,numpy as np
from pathlib import Path
from mathutils import Vector,Matrix
root=Path.cwd();out=root/os.environ.get('REVIEW_OUT','outputs/rectangular-photo-refinement/review');out.mkdir(parents=True,exist_ok=True);bs=json.loads((root/'work/buildings/v2/inventory.json').read_text())['buildings'];poses=json.loads((root/'work/buildings/visibility/poses.json').read_text());s=bpy.context.scene;parent=next(c for c in bpy.data.collections if c.name.startswith('11 ARCHITECTURE'));cols={c.name.split(' | ')[0]:c for c in parent.children};bs=[b for b in bs if b['id'] in cols];IDs=set(filter(None,os.environ.get('REVIEW_IDS','').split(',')))
...
s.render.engine='CYCLES';s.cycles.device='CPU';s.cycles.samples=int(os.environ.get('REVIEW_SAMPLES','32'));s.cycles.use_denoising=True;s.render.resolution_percentage=100;s.render.image_settings.file_format='PNG';s.view_settings.view_transform='AgX';s.view_settings.look='AgX - Medium High Contrast';s.view_settings.exposure=.1;s.view_layers[0].material_override=None
camdata=bpy.data.cameras.new('V2 review camera');cam=bpy.data.objects.new(camdata.name,camdata);s.collection.objects.link(cam);s.camera=cam;camdata.clip_end=5000;manifest=[]
def project(q,p):
...
for b in bs:
 ...
 print('V2_REVIEW_RENDERED',b['id'],flush=True)
(out/'source_view_manifest.json').write_text(json.dumps(manifest,indent=2))
