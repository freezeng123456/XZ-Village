...
root=Path.cwd();out=root/'outputs/building-quality';s=bpy.context.scene;s.render.engine='CYCLES';s.cycles.samples=32;s.cycles.use_denoising=True;s.render.resolution_x=2400;s.render.resolution_y=1600;s.render.resolution_percentage=100
...
bs=json.loads((root/'work/buildings/visibility/architecture_visibility.json').read_text())['buildings'];pts=np.vstack([b['roof_world'] for b in bs]);lo=pts.min(0);hi=pts.max(0);center=Vector((lo+hi)/2);center.z=4
...
s.camera=bpy.data.objects['Video 044.00s | recovered'];s.render.resolution_x=1600;s.render.resolution_y=900;s.render.filepath=str(out/'Architecture_44s.png');bpy.ops.wm.save_as_mainfile(filepath=str(out/'Village_Building_Candidates.blend'),compress=True)
