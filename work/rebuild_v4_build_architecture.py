...
P=Path('work/rebuild4/architecture').resolve();d=json.loads((P/os.getenv('V4_GEOMETRY','pilot_mesh_input.json')).read_text());profiles={p['id']:p for p in json.loads((P/os.getenv('V4_PROFILES','pilot_profiles.json')).read_text())['entries']};OUT=P/os.getenv('V4_OUTPUT','pilot_detailed');OUT.mkdir(exist_ok=True)
...
class Geometry:
 ...
 def done(self):
  ...
  self.col['source_id']=self.id;self.col['observed_and_inferred_detail_ledger']=os.getenv('V4_PROFILES','pilot_profiles.json');self.col['opening_count']=self.openings;self.col['inferred_opening_count']=self.inferred;return dict(id=self.id,openings=self.openings,inferred_openings=self.inferred,components=self.components,vertices=sum(len(o.data.vertices) for o in self.col.objects if o.type=='MESH'))
exec((Path('work')/'rebuild_v4_roof_details.py').read_text())
exec((Path('work')/'rebuild_v4_distinctive_details.py').read_text())
exec((Path('work')/'rebuild_v4_special_buildings_geometry.py').read_text())
exec((Path('work')/'rebuild_v4_terrace_geometry.py').read_text())
def building(b,p):
...
if g:
 ...
 if envpath.exists() and os.getenv('V4_GEOMETRY','').startswith('village'):
  ...
  exec((Path('work')/'rebuild_v4_build_environment.py').read_text());add_environment(env)
