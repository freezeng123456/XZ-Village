...
root=Path.cwd();out=root/'outputs/building-quality';data=json.loads((root/'work/buildings/visibility/architecture_visibility.json').read_text())['buildings'];result={'checks':{},'buildings':[]};parent=next(c for c in bpy.data.collections if c.name.startswith('11 ARCHITECTURE'))
