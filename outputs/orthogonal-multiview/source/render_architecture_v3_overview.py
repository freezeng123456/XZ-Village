...
root=Path.cwd();out=root/'outputs/orthogonal-multiview';s=bpy.context.scene;s.render.engine='CYCLES';s.cycles.samples=32;s.cycles.use_denoising=True;s.render.resolution_x=2400;s.render.resolution_y=1600;s.render.resolution_percentage=100
...
bs=json.loads((root/'work/buildings/v3/inventory.json').read_text())['buildings'];pts=np.vstack([b['roof_world'] for b in bs]);lo=pts.min(0);hi=pts.max(0);center=Vector((lo+hi)/2);center.z=4
...
for name,ids in [('Detail_Modern',['B007','B008','B009','B010']),('Detail_Tile',['O001','O002','O003','O007','O008','O011','O012']),('Detail_Solar',['A004']),('Detail_Construction',['B013'])]:
