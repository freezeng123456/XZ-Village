...
root=Path.cwd();out=root/'outputs/orthogonal-multiview';data=json.loads((root/'work/buildings/v3/inventory.json').read_text())['buildings'];result={'checks':{},'buildings':[]};parent=next(c for c in bpy.data.collections if c.name.startswith('11 ARCHITECTURE'))
...
for b in data:
 ...
 finite=True;zero=0;vertices=0;wall_bounds=True;local=np.array(b.get('plan_polygon',[[0,0],[1,0],[1,1],[0,1]]))*[b['rectangle']['width'],b['rectangle']['depth']];ob=next(o for o in c.objects if o.type=='MESH');footprint=np.array([tuple(ob.matrix_world@__import__('mathutils').Vector((x,y,0))) for x,y in local]);base=b['base_height'];roof=b['roof_height']
 for o in c.objects:
  if o.type=='MESH':
 ...
 for o in c.objects:
  ...
  for comp in json.loads(o.get('original_components','[]')):
   if 'Ground floor orthogonal slab' not in comp['name']:continue
   ix=set(i for poly in list(o.data.polygons)[comp['first_polygon']:comp['first_polygon']+comp['polygon_count']] for i in poly.vertices);v=np.array([o.data.vertices[i].co[:] for i in ix]);saved=np.unique(np.round(v[:,:2],4),axis=0);edges0=np.roll(local,-1,axis=0)-local;normals0=np.column_stack([edges0[:,1],-edges0[:,0]])/np.linalg.norm(edges0,axis=1)[:,None];expected=np.unique(np.round(local-.01*(normals0+np.roll(normals0,1,axis=0)),4),axis=0);edges=np.roll(local,-1,axis=0)-local;slab_ok=bool(saved.shape==expected.shape and np.allclose(saved,expected,atol=1e-4) and all(abs(float(a@z))<1e-6 for a,z in zip(edges,np.roll(edges,-1,axis=0))))
 checks={'saved_ground_slab_matches_orthogonal_plan':slab_ok,'wall_panels_within_building_envelope':wall_bounds,'finite_mesh_coordinates':finite,'has_floor':any('Ground floor orthogonal slab' in n for n in names),'has_wall_panels':any('Wall panel' in n for n in names),'has_actual_openings':int(c.get('opening_count',0))>0,'has_window_frames_or_unfinished_openings':b['type']=='construction' or b['id']=='B013' or any('frame' in n.lower() for n in names),'has_roof_detail':any(any(k in n for k in ['Roof slab expansion','Roof slab transverse','Roof drain','Individual overlapping','Standing roof seam','glazed roof','Glazed tile rib','column reinforcement']) for n in names),'has_drainage':any('downpipe' in n for n in names),'has_materials':all(len(o.data.materials)>0 for o in c.objects if o.type in ('MESH','CURVE')),'no_degenerate_mesh_faces':zero==0}
...
result['world_plan_polygons']={c.name.split(' | ')[0]:[tuple(next(o for o in c.objects if o.type=='MESH').matrix_world@__import__('mathutils').Vector((x,y,0))) for x,y in json.loads(c['local_plan_polygon'])] for c in cols.values()};result['saved_file']=bpy.data.filepath;result['passed']=all(result['checks'].values());(out/'validation.json').write_text(json.dumps(result,ensure_ascii=False,indent=2));print(json.dumps(dict(checks=result['checks'],counts=result['counts'],failed=[b for b in result['buildings'] if not b['passed']]),ensure_ascii=False,indent=2))
