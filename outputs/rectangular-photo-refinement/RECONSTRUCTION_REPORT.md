...
本轮根据“住宅方正，整体颜色和纹理应与照片对齐”的要求重建建筑。按最新要求，交付模型已移除所有照片地面和立面照片覆层；原视频画面只用于独立分栏对照。保留原红框建筑 B001，重新生成其余 **297 个编号单元**；编号包含主屋、连接侧翼与小型附属房，不等同于独立住户数。
...
| 有高度重叠的住宅主体之间穿插 | 0 对 |
| 原红框建筑 | 删除 7 个照片覆层，其余 674 个对象几何保持一致 |
| 新建筑对象 | 6,312 |
| 新建筑网格顶点 | 1,917,734 |
| 实际开洞（包括门、窗与阳台） | 5,010 |
| 外部链接库 | 0 |
