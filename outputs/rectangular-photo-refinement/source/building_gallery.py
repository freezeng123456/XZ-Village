...
root=Path.cwd();out=root/os.environ.get('GALLERY_OUT','outputs/building-quality/review');out.mkdir(parents=True,exist_ok=True)
original=bpy.context.scene
bs=json.loads((root/os.environ.get('ARCHITECTURE_SPEC','work/buildings/visibility/architecture_visibility.json')).read_text())['buildings']
...
scene=bpy.data.scenes.new('Per-building review');scene.world=original.world.copy();scene.render.engine='CYCLES';scene.cycles.device='CPU';scene.cycles.samples=12;scene.cycles.use_denoising=True;scene.render.resolution_x=2400;scene.render.resolution_y=1200;scene.render.resolution_percentage=100;scene.render.image_settings.file_format='PNG';scene.render.film_transparent=False;scene.view_settings.view_transform='AgX';scene.view_settings.look='AgX - Medium High Contrast';scene.view_settings.exposure=float(os.environ.get('GALLERY_EXPOSURE','.5'))
