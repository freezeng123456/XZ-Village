...
root=Path.cwd();out=root/'outputs/rectangular-photo-refinement';v=json.loads((out/'validation.json').read_text());review=json.loads((out/'visual_review.json').read_text());ids={b['id'] for b in v['buildings']}
...
names={b['id']:b['name'] for b in json.loads((root/'work/buildings/v2/inventory.json').read_text())['buildings']}
s=bpy.context.scene
...
for c in bpy.data.collections:
 if c.name.split(' | ')[0] in ids:
  bid=c.name.split(' | ')[0];c.name=bid+' | '+names[bid];c['status']='Rectangular source-refined architecture; visible and opposite views inspected; unobserved details inferred'
...
for text in list(bpy.data.texts):
 if text.name.startswith('ARCHITECTURE |'):bpy.data.texts.remove(text)
...
parent=next(c for c in bpy.data.collections if c.name.startswith('11 ARCHITECTURE'));parent.name='11 ARCHITECTURE | rectangular source-refined buildings'
...
v['counts']['packed_images']=sum(bool(i.packed_file) for i in bpy.data.images);v['saved_file']=str(out/'Village_Rectangular_Refined.blend');ledger=bpy.data.texts.get('ARCHITECTURE | reviewed building ledger');ledger.clear();ledger.write(json.dumps(v,ensure_ascii=False,indent=2))
bpy.context.preferences.filepaths.save_version=0;bpy.ops.wm.save_as_mainfile(filepath=str(out/'Village_Rectangular_Refined.blend'),compress=True);print('REVIEWED_LOCAL_RELEASE_SAVED',len(ids)+1)
